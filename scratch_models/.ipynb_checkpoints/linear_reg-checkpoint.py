...
class LinearRegression:
    ...
    def fit(self):
        ...
        self.msr = self.ssr / (
            self.df_model - 1 if self.intercept else self._n[1]
        )
    ...
    def diagnostics(self, X=None):
        if X:
            ...
            sm.qqplot(self.residuals, line="q")

            # return sns.scatterplot(x=self.X @ self.coefs, y=self.residuals)
